...
export class EggGachaUiHandler extends MessageUiHandler {
  ...
  async pull(pullCount = 0): Promise<void> {
    ...
    const saveSuccess = await (globalScene.currentBattle && playerIndex === 0
      ? globalScene.gameData.saveAll(true, true, true)
      : globalScene.savePlayerSystemSave(playerIndex)
    ).then(success => {
      if (!success) {
  ...
  consumeVouchers(voucherType: VoucherType, count: number): void {
    const voucherCounts = globalScene.getPlayerGameData(this.getPlayerIndex()).voucherCounts;
    voucherCounts[voucherType] = Math.max(
      voucherCounts[voucherType] - count,
      0,
    );
    globalScene.savePlayerSystemSaveLocal(this.getPlayerIndex());
    this.updateVoucherCounts();
  }
  ...
  updateVoucherCounts(): void {
    ...
    this.voucherCountLabels.forEach((label, type) => {
      label.setText(voucherCounts[type].toString());
    });
  }
  ...
  private handleVoucherSelectAction(cursor: number): boolean | undefined {
    ...
    const gameData = globalScene.getPlayerGameData(this.getPlayerIndex());
    if (!freePulls && gameData.eggs.length + pulls > 99) {
      errorKey = "egg:tooManyEggs";
    } else if (!freePulls && gameData.voucherCounts[voucherType] < vouchersConsumed) {

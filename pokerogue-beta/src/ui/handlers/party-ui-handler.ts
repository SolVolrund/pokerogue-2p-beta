...
export class PartyUiHandler extends MessageUiHandler {
  ...
  private getPlayerIndex(): PlayerIndex {
    if (globalScene.twoPlayerMode && this.fieldIndex >= 0) {
      return globalScene.getPlayerIndexForFieldSlot(this.fieldIndex);
    }

    return globalScene.activePlayerIndex;
  }

  private getParty(): PlayerPokemon[] {
    return globalScene.getPlayerParty(this.getPlayerIndex());
  }

  private getActivePartySlotCount(): number {
    return globalScene.twoPlayerMode ? 1 : globalScene.currentBattle.getBattlerCount();
  }

  private usesDoublePartyLayout(): boolean {
    return globalScene.currentBattle.double && this.getActivePartySlotCount() > 1;
  }

  public static FilterItemMaxStacks = (pokemon: PlayerPokemon, modifier: PokemonHeldItemModifier) => {
    const matchingModifier = globalScene.findModifierForPokemon(
  ...
  show(args: any[]): boolean {
    ...
    const doublePartyLayout = this.usesDoublePartyLayout();
    if (this.isItemManageMode()) {
      this.partyBg.setTexture(`party_bg${doublePartyLayout ? "_double_manage" : ""}`);
    } else {
      this.partyBg.setTexture(`party_bg${doublePartyLayout ? "_double" : ""}`);
    }
  ...
  private processReleaseOption(pokemon: Pokemon): boolean {
    ...
    if (this.cursor >= this.getActivePartySlotCount() || !pokemon.isAllowedInBattle()) {
  ...
  private processTransferOption(): boolean {
    ...
    if (this.transferCursor !== this.cursor) {
      if (this.transferAll) {
        this.getTransferrableItemsFromPokemon(this.getParty()[this.transferCursor]).forEach(
          (_, i, array) => {
  ...
  private processRememberMoveModeUpDownInput(button: Button.UP | Button.DOWN) {
    ...
    const option = this.options[this.optionsCursor];
    const pokemon = this.getParty()[this.cursor];
    const learnableMoves = pokemon.getLearnableLevelMoves();
    const moveId = learnableMoves[option]?.[1];
  ...
  private getFilterResult(option: number, pokemon: PlayerPokemon): string | null {
    ...
    if (option !== PartyOption.TRANSFER && option !== PartyOption.SPLICE) {
      ...
      filterResult = (this.selectFilter as PokemonModifierTransferSelectFilter)(
        pokemon,
        this.getTransferrableItemsFromPokemon(this.getParty()[this.transferCursor])[
          this.transferOptionCursor
        ],
      );
    }
      ...
      if (
        ...
        const itemModifiers = globalScene.findModifiersForPlayer(
          m =>
            m.is("PokemonHeldItemModifier")
            && m.isTransferable
            && m.pokemonId === this.getParty()[this.cursor].id,
          this.getPlayerIndex(),
        ) as PokemonHeldItemModifier[];
  ...
  private processPartyDirectionalInput(button: Button.UP | Button.DOWN | Button.LEFT | Button.RIGHT): boolean {
    ...
    const battlerCount = this.getActivePartySlotCount();
    const doublePartyLayout = this.usesDoublePartyLayout();
    ...
    switch (button) {
      ...
      case Button.UP:
        if (this.isItemManageMode()) {
          if (this.cursor === 1) {
            success = this.setCursor(doublePartyLayout ? 0 : 7);
            break;
          }
          if (this.cursor === 2) {
            success = this.setCursor(doublePartyLayout ? 7 : 1);
            break;
          }
          if (this.cursor === 6) {
            success = this.setCursor(slotCount <= battlerCount ? 7 : slotCount - 1);
            break;
          }
          if (this.cursor === 7) {
            success = this.setCursor(doublePartyLayout && slotCount > 1 ? 1 : 0);
            break;
          }
        }
      ...
      case Button.DOWN:
        if (this.isItemManageMode()) {
          if (this.cursor === 0) {
            success = this.setCursor(doublePartyLayout && slotCount > 1 ? 1 : 7);
            break;
          }
          if (this.cursor === 1) {
            success = this.setCursor(doublePartyLayout ? 7 : slotCount > 2 ? 2 : 6);
            break;
          }
          if (this.cursor === 7) {
            success = this.setCursor(
              slotCount > battlerCount ? battlerCount : 6,
            );
            break;
          }
        }
  ...
  updateOptions(): void {
    ...
    switch (this.partyUiMode) {
      ...
      case PartyUiMode.POST_BATTLE_SWITCH:
        if (this.cursor >= this.getActivePartySlotCount()) {
  ...
  private updateOptionsWindow(): void {
    ...
    for (let o = 0; o < this.options.length; o++) {
      ...
      if (
        ...
        if (this.transferQuantitiesMax[option] === itemModifier.getMaxHeldItemCount(pokemon)) {
  ...
  doRelease(slotIndex: number): void {
    ...
    this.showText(
      ...
      () => {
        this.clearPartySlots();
        globalScene.removePartyMemberModifiers(slotIndex, this.getPlayerIndex());
        const releasedPokemon = this.getParty().splice(slotIndex, 1)[0];
        releasedPokemon.destroy();
        this.populatePartySlots();
        if (this.cursor >= this.getParty().length) {
  ...
  getFormChangeItemsModifiers(pokemon: Pokemon) {
    let formChangeItemModifiers = globalScene.findModifiersForPlayer(
...
class PartySlot extends Phaser.GameObjects.Container {
  ...
  constructor(
    ...
    tmMoveId: MoveId,
    activePartySlotCount = globalScene.currentBattle.getBattlerCount(),
  ) {
    const isBenched = slotIndex >= activePartySlotCount;
    const isDoubleBattle = activePartySlotCount > 1;

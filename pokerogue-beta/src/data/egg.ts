...
export class Egg {
  ...
  private ownerPlayerIndex: PlayerIndex;
  ...
  constructor(eggOptions?: IEggOptions) {
    this.ownerPlayerIndex = eggOptions?.playerIndex ?? globalScene.activePlayerIndex;
    ...
    const generateEggProperties = (eggOptions?: IEggOptions) => {
      ...
      if (eggOptions?.pulled) {
        ...
        this.addEggToGameData(this.ownerPlayerIndex);
      }
    };
  ...
  public addEggToGameData(playerIndex: PlayerIndex = globalScene.activePlayerIndex): void {
    globalScene.getPlayerGameData(playerIndex).eggs.push(this);
  ...
  private getOwnerGameData() {
    return globalScene.getPlayerGameData(this.ownerPlayerIndex);
  }
  ...
  private rollSpecies(): SpeciesId | null {
    ...
    const gameData = this.getOwnerGameData();

    if (gameData.unlockPity[this.tier] >= 9) {
      const lockedPool = speciesPool.filter(
        s =>
          !gameData.dexData[s.speciesId].caughtAttr
          && !gameData.eggs.some(e => e.species === s.speciesId),
      );
    ...
    if (
      gameData.dexData[species].caughtAttr
      || gameData.eggs.some(e => e.species === species)
    ) {
      gameData.unlockPity[this.tier] = Math.min(gameData.unlockPity[this.tier] + 1, 10);
    } else {
      gameData.unlockPity[this.tier] = 0;
    }
  ...
  private checkForPityTierOverrides(): void {
    const tierValueOffset =
      this._sourceType === EggSourceType.GACHA_LEGENDARY ? GACHA_LEGENDARY_UP_THRESHOLD_OFFSET : 0;
    const gameData = this.getOwnerGameData();
    gameData.eggPity[EggTier.RARE] += 1;
    gameData.eggPity[EggTier.EPIC] += 1;
    gameData.eggPity[EggTier.LEGENDARY] += 1 + tierValueOffset;
    // These numbers are roughly the 80% mark. That is, 80% of the time you'll get an egg before this gets triggered.
    if (
      gameData.eggPity[EggTier.LEGENDARY] >= EGG_PITY_LEGENDARY_THRESHOLD
      && this._tier === EggTier.COMMON
    ) {
      this._tier = EggTier.LEGENDARY;
    } else if (gameData.eggPity[EggTier.EPIC] >= EGG_PITY_EPIC_THRESHOLD && this._tier === EggTier.COMMON) {
      this._tier = EggTier.EPIC;
    } else if (gameData.eggPity[EggTier.RARE] >= EGG_PITY_RARE_THRESHOLD && this._tier === EggTier.COMMON) {
      this._tier = EggTier.RARE;
    }
    gameData.eggPity[this._tier] = 0;
  }

  private increasePullStatistic(): void {
    const gameStats = this.getOwnerGameData().gameStats;
    gameStats.eggsPulled++;
    if (this.isManaphyEgg()) {
      gameStats.manaphyEggsPulled++;
      this._hatchWaves = this.getEggTierDefaultHatchWaves(EggTier.EPIC);
      return;
    }
    switch (this.tier) {
      case EggTier.RARE:
        gameStats.rareEggsPulled++;
        break;
      case EggTier.EPIC:
        gameStats.epicEggsPulled++;
        break;
      case EggTier.LEGENDARY:
        gameStats.legendaryEggsPulled++;
        break;
    }
  }

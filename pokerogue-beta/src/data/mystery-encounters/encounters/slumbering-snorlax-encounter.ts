...
function setOwnedLeftoversRewards(fillRemaining: boolean): void {
  const playerIndexes = (globalScene.twoPlayerMode ? [0, 1] : [globalScene.activePlayerIndex]) as PlayerIndex[];
...
export const SlumberingSnorlaxEncounter: MysteryEncounter = MysteryEncounterBuilder.withEncounterType(
  ...
  .withOnInit(() => {
    ...
    const pokemonConfigs = Array.from({ length: globalScene.twoPlayerMode ? 2 : 1 }, () => getSnorlaxPokemonConfig());
    ...
    encounter.spriteConfigs = pokemonConfigs.map((_, index) => ({
      species: SpeciesId.SNORLAX,
      spriteKey: "",
      fileRoot: "",
      hasShadow: true,
      ...
      scale: 1.25,
      repeat: true,
      ...(globalScene.twoPlayerMode ? { x: -18 + index * 36 } : {}),
      y: 5,
    }));
  ...
  .withSimpleOption(
    ...
    async () => {
      ...
      if (globalScene.twoPlayerMode) {
        globalScene.phaseManager.unshiftNew("PartyHealPhase", true, 1);
        globalScene.phaseManager.unshiftNew("PartyHealPhase", true, 0);
      } else {
  ...
  .withOption(
    MysteryEncounterOptionBuilder.newOptionWithMode(MysteryEncounterOptionMode.DISABLED_OR_SPECIAL)
      ...
      .withOptionPhase(async () => {
        ...
        setOwnedLeftoversRewards(false);
        // Snorlax exp to Pokemon that did the stealing
        const primaryPokemon = instance.options[2].primaryPokemon!;
        const playerIndex = globalScene.getPlayerIndexForPokemon(primaryPokemon) ?? globalScene.activePlayerIndex;
        setEncounterExp(primaryPokemon.id, getPokemonSpecies(SpeciesId.SNORLAX).baseExp, true, playerIndex);
        leaveEncounterWithoutBattle();
      })
      .build(),
  )

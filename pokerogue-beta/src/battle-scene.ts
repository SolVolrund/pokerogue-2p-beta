...
function createDefaultPokeballCounts(): PokeballCounts {
  ...
  pokeballCounts[PokeballType.POKEBALL] = 5;
  return pokeballCounts;
}

function createInitialPokeballCounts(): PokeballCounts {
  if (activeOverrides.POKEBALL_OVERRIDE.active) {
    return { ...activeOverrides.POKEBALL_OVERRIDE.pokeballs };
  }

  return createDefaultPokeballCounts();
}
...
function getTwoPlayerPartySize(): 3 | 6 {
...
function getTwoPlayerPlayerCount(): MultiplayerPlayerCount {
  if (typeof window === "undefined") {
    return 2;
  }

  return new URLSearchParams(window.location.search).get("twoPlayerPlayerCount") === "3" ? 3 : 2;
}
...
export class BattleScene extends SceneBase {
  ...
  launchBattle() {
    ...
    this.money = 0;
    this.pokeballCounts = createInitialPokeballCounts();
    this.modifiers = [];
    this.enemyModifiers = [];
  ...
  private syncLegacyStateForActivePlayer(): void {
    if (!this.twoPlayerMode) {
      return;
    }

    const activePlayer = this.players[this.activePlayerIndex];
    this.party = activePlayer.party;
    this.money = activePlayer.money;
    this.pokeballCounts = activePlayer.pokeballCounts;
    this.modifiers = activePlayer.modifiers;
  }
  ...
  private initializePlayerSystemSaves(): void {
    ...
    this.systemSaves = getPlayerIndexes(this.multiplayerPlayerCount).map(playerIndex => {
      if (localSeat !== "host-and-guest-local" && playerIndex === localSeat) {
        return localSystemSave;
      }

      if (localSeat === "host-and-guest-local" && playerIndex === 0) {
        return localSystemSave;
      }
  ...
  public configureTwoPlayerMode(
    ...
    const localSystemSave = this.getLocalPlayerSystemSave();
    if (this.systemSaves && this.isLocalSystemSaveOwner(0)) {
    ...
    if (enabled) {
      if (!this.systemSaves || this.systemSaves.length < this.multiplayerPlayerCount) {
        this.initializePlayerSystemSaves();
      }
  ...
  private loadGuestSystemSave(): GameData {
    const guestSystemSave = localStorage.getItem(TWO_PLAYER_GUEST_SYSTEM_SAVE_KEY);
    if (!guestSystemSave) {
      return new GameData(true);
    }

    try {
      return GameData.fromRawSystem(guestSystemSave, false, 1);
    } catch (err) {
      console.warn("Failed to load 2P guest profile; creating a fresh guest profile.", err);
      return new GameData(true);
    }
  }
  ...
  public applyTwoPlayerProfileSnapshot(profileSnapshot: TwoPlayerProfileSnapshot): boolean {
    ...
    try {
      gameData = GameData.fromRawSystem(profileSnapshot.systemSave, false, profileSnapshot.playerIndex);
    } catch (err) {
      console.warn(`Failed to import 2P profile snapshot for player ${profileSnapshot.playerIndex}.`, err);
      return false;
    }

    const systemSaves = this.ensurePlayerSystemSaves();
  ...
  private refreshTwoPlayerProfileSlotsReady(): void {
    ...
    this.twoPlayerProfileSlotsReady = ([0, 1, 2] as PlayerIndex[]).map(playerIndex => {
      ...
      if (localSeat === "host-and-guest-local") {
        return playerIndex === 0 ? this.localPlayerSystemSaveLoaded : true;
      }
  ...
  public isTwoPlayerProfileExchangeComplete(): boolean {
    return !this.twoPlayerMode || this.twoPlayerProfileSlotsReady.every(Boolean);
  }

  public waitForTwoPlayerProfileExchange(timeoutMs = 15000): Promise<boolean> {
    if (this.isTwoPlayerProfileExchangeComplete()) {
      return Promise.resolve(true);
    }

    return new Promise(resolve => {
      const resolver = (ready: boolean) => {
        clearTimeout(timeout);
        resolve(ready);
      };
      const timeout = setTimeout(() => {
        this.twoPlayerProfileReadyResolvers = this.twoPlayerProfileReadyResolvers.filter(r => r !== resolver);
        resolve(this.isTwoPlayerProfileExchangeComplete());
      }, timeoutMs);

      this.twoPlayerProfileReadyResolvers.push(resolver);
    });
  }
  ...
  private resolveTwoPlayerProfileReadyWaiters(): void {
    ...
    const resolvers = this.twoPlayerProfileReadyResolvers.splice(0, this.twoPlayerProfileReadyResolvers.length);
  ...
  public getLocalSystemSaveSeat(): PlayerIndex | "host-and-guest-local" {
    ...
    return this.twoPlayerLocalInputSeat;
  }
  ...
  public isLocalSystemSaveOwner(playerIndex: PlayerIndex): boolean {
    ...
    const localSeat = this.getLocalSystemSaveSeat();
    return localSeat === "host-and-guest-local" ? playerIndex === 0 : localSeat === playerIndex;
  }

  public getLocalPlayerGameData(): GameData {
    if (!this.twoPlayerMode) {
  ...
  private saveSessionSystemSaveLocal(playerIndex: PlayerIndex, gameData = this.getPlayerGameData(playerIndex)): void {
  ...
  private syncSinglePlayerStateReference(): void {
    if (this.twoPlayerMode) {
      return;
    }

    this.players[0].party = this.party;
    this.players[0].money = this.money;
    this.players[0].pokeballCounts = this.pokeballCounts;
    this.players[0].modifiers = this.modifiers;
  }
  ...
  public getPlayerState(playerIndex: PlayerIndex = this.activePlayerIndex): PlayerRunState {
    if (this.twoPlayerMode) {
  ...
  public isComputerPartnerPlayer(playerIndex: PlayerIndex): boolean {
    return (
  ...
  public getComputerPartnerKey(playerIndex: PlayerIndex = 1): ComputerPartnerKey {
    return playerIndex === 1 ? this.computerPartnerKey : (this.computerPartnerKeys[playerIndex] ?? "alex");
  }

  public setComputerPartnerKey(playerIndex: PlayerIndex, key: ComputerPartnerKey): void {
    this.computerPartnerKeys[playerIndex] = key;
    if (playerIndex === 1) {
    ...
    if (playerIndex === 1) {
      ...
      this.twoPlayerGuestTrainerSprite = profile.trainerSprite ?? fallbackSprite;
  ...
  public getPlayerMoney(playerIndex: PlayerIndex = this.activePlayerIndex): number {
    return this.getPlayerState(playerIndex).money;
  }

  public setPlayerMoney(amount: number, playerIndex: PlayerIndex = this.activePlayerIndex): void {
  ...
  public getPlayerPokeballCounts(playerIndex: PlayerIndex = this.activePlayerIndex): PokeballCounts {
  ...
  public getPlayerIndexForPokemon(pokemon: Pokemon): PlayerIndex | undefined {
    ...
    const playerIndex = this.players.findIndex(player => player.party.includes(pokemon as PlayerPokemon));
  ...
  public applyTwoPlayerTitleStart(titleStart: TwoPlayerTitleStart): void {
    if (!this.twoPlayerMode) {
      return;
    }
  ...
  public setTwoPlayerTitleStartHandler(handler: ((titleStart: TwoPlayerTitleStart) => void) | undefined): void {
    this.twoPlayerTitleStartHandler = handler;
  }

  public clearPendingTwoPlayerTitleStart(titleStart: TwoPlayerTitleStart): void {
    if (this.pendingTwoPlayerTitleStart === titleStart) {
      this.pendingTwoPlayerTitleStart = undefined;
    }
  }

  public consumePendingTwoPlayerTitleStart(): TwoPlayerTitleStart | undefined {
    const titleStart = this.pendingTwoPlayerTitleStart;
    this.pendingTwoPlayerTitleStart = undefined;
    return titleStart;
  }
  ...
  reset(clearScene = false, clearData = false, reloadI18n = false): void {
    ...
    const playerPokemon = this.twoPlayerMode ? this.players.flatMap(player => player.party) : this.getPlayerParty();
  ...
  addMoney(amount: number): void {
    this.addMoneyForPlayer(amount);
  }

  addMoneyForPlayer(amount: number, playerIndex: PlayerIndex = this.activePlayerIndex, showUpdate = true): void {
    const money = Math.min(this.getPlayerMoney(playerIndex) + amount, Number.MAX_SAFE_INTEGER);
    this.setPlayerMoney(money, playerIndex);
  ...
  addModifier(
    ...
    if (modifier instanceof PersistentModifier) {
      ...
      if ((modifier as PersistentModifier).add(playerModifiers, !!virtual)) {
      ...
      for (const rm of modifiersToRemove) {
        this.removeModifier(rm, false, playerIndex);
      }

      if (!ignoreUpdate && !virtual) {
        this.updateModifiers(true, instant, playerIndex);
      }
        ...
        for (const p in party) {
          ...
          if (modifier instanceof PokemonHpRestoreModifier) {
            if ((modifier as PokemonHpRestoreModifier).fainted) {
              ...
              this.applyModifiersForPlayer(HealingBoosterModifier, playerIndex, hpRestoreMultiplier);
  ...
  tryTransferHeldItemModifier(
    ...
    if (!removeOld || !source || this.removeModifier(itemModifier, source.isEnemy(), sourcePlayerIndex)) {
      const addModifier = () => {
        if (!matchingModifier || this.removeModifier(matchingModifier, target.isEnemy(), targetPlayerIndex)) {
          if (target.isPlayer()) {
            this.addModifier(newItemModifier, ignoreUpdate, playSound, false, instant, undefined, targetPlayerIndex);
            if (source && itemLost) {
      ...
      if (source && source.isPlayer() !== target.isPlayer() && !ignoreUpdate) {
        this.updateModifiers(source.isPlayer(), instant, sourcePlayerIndex);
        addModifier();
      } else {
        addModifier();
      }
  ...
  removePartyMemberModifiers(partyMemberIndex: number, playerIndex: PlayerIndex = this.activePlayerIndex): Promise<void> {
    return new Promise(resolve => {
      ...
      const modifiersToRemove = modifiers.filter(
  ...
  // TODO: Document this
  updateModifiers(player = true, instant?: boolean, playerIndex: PlayerIndex = this.activePlayerIndex): void {
    const modifiers = this.getModifierList(player, playerIndex);
  ...
  hasModifier(modifier: PersistentModifier, enemy = false, playerIndex: PlayerIndex = this.activePlayerIndex): boolean {
    const modifiers = enemy ? this.enemyModifiers : this.getPlayerModifiers(playerIndex);
  ...
  getModifiers<T extends PersistentModifier>(
    ...
    return this.getModifierList(player, playerIndex).filter((m): m is T => m instanceof modifierType);
  }

  getModifiersForPlayer<T extends PersistentModifier>(modifierType: Constructor<T>, playerIndex: PlayerIndex): T[] {
    return this.getModifiers(modifierType, true, playerIndex);
  }
  ...
  findModifiers(
    ...
    return this.getModifierList(isPlayer, playerIndex).filter(modifierFilter);
  }

  findModifiersForPlayer(modifierFilter: ModifierPredicate, playerIndex: PlayerIndex): PersistentModifier[] {
    return this.findModifiers(modifierFilter, true, playerIndex);
  }

  findModifiersForPokemon(modifierFilter: ModifierPredicate, pokemon: Pokemon): PersistentModifier[] {
    const playerIndex = pokemon.isPlayer() ? this.getPlayerIndexForPokemon(pokemon) : undefined;
    return this.findModifiers(modifierFilter, pokemon.isPlayer(), playerIndex);
  }
  ...
  findModifier(
    ...
    return this.getModifierList(player, playerIndex).find(modifierFilter);
  }

  findModifierForPlayer(modifierFilter: ModifierPredicate, playerIndex: PlayerIndex): PersistentModifier | undefined {
    return this.findModifier(modifierFilter, true, playerIndex);
  }

  findModifierForPokemon(modifierFilter: ModifierPredicate, pokemon: Pokemon): PersistentModifier | undefined {
    const playerIndex = pokemon.isPlayer() ? this.getPlayerIndexForPokemon(pokemon) : undefined;
    return this.findModifier(modifierFilter, pokemon.isPlayer(), playerIndex);
  }
  ...
  applyShuffledModifiers<T extends PersistentModifier>(
    ...
    let modifiers = this.getModifierList(player).filter(
  ...
  applyModifiers<T extends PersistentModifier>(
    ...
    const modifiers = this.getModifierList(player).filter(
      (m): m is T => m instanceof modifierType && m.shouldApply(...args),
    );
    return this.applyModifiersInternal(modifiers, player, args);
  }

  applyModifiersForPlayer<T extends PersistentModifier>(
    modifierType: Constructor<T>,
    playerIndex: PlayerIndex,
    ...args: Parameters<T["apply"]>
  ): T[] {
    const modifiers = this.getPlayerModifiers(playerIndex).filter(
      (m): m is T => m instanceof modifierType && m.shouldApply(...args),
    );
    return this.applyModifiersInternal(modifiers, true, args);
  }

  applyModifiersForPokemon<T extends PersistentModifier>(
    modifierType: Constructor<T>,
    pokemon: Pokemon,
    ...args: Parameters<T["apply"]>
  ): T[] {
    const playerIndex = pokemon.isPlayer() ? this.getPlayerIndexForPokemon(pokemon) : undefined;
    if (pokemon.isPlayer() && playerIndex !== undefined) {
      return this.applyModifiersForPlayer(modifierType, playerIndex, ...args);
    }
  ...
  applyModifierForPlayer<T extends PersistentModifier>(
    modifierType: Constructor<T>,
    playerIndex: PlayerIndex,
    ...args: Parameters<T["apply"]>
  ): T | null {
  ...
  applyModifierForPokemon<T extends PersistentModifier>(
    modifierType: Constructor<T>,
    pokemon: Pokemon,
    ...args: Parameters<T["apply"]>
  ): T | null {
    const playerIndex = pokemon.isPlayer() ? this.getPlayerIndexForPokemon(pokemon) : undefined;
    if (pokemon.isPlayer() && playerIndex !== undefined) {
      return this.applyModifierForPlayer(modifierType, playerIndex, ...args);
    }
    return this.applyModifier(modifierType, pokemon.isPlayer(), ...args);
  }
  ...
  validateVoucher(voucher: Voucher, args?: unknown[]): boolean {
    return this.validateVoucherForPlayer(voucher, this.activePlayerIndex, args);
  }

  validateVoucherForPlayer(voucher: Voucher, playerIndex: PlayerIndex, args?: unknown[]): boolean {
    const gameData = this.getPlayerGameData(playerIndex);
    if (!Object.hasOwn(gameData.voucherUnlocks, voucher.id) && voucher.validate(args)) {
      gameData.voucherUnlocks[voucher.id] = Date.now();
      this.ui.achvBar.showAchv(voucher);
      gameData.voucherCounts[voucher.voucherType]++;
      this.savePlayerSystemSaveLocal(playerIndex);
      return true;
    }
  ...
  applyPartyExp(
    ...
    if (this.twoPlayerMode && playerIndex === undefined) {
      ...
      this.setActivePlayerIndex(previousPlayerIndex);
      return;
    }

    const expPlayerIndex = playerIndex ?? this.activePlayerIndex;
    if (this.twoPlayerMode) {
      this.setActivePlayerIndex(expPlayerIndex);
    }
      ...
      for (const partyMember of nonFaintedPartyMembers) {
        ...
        if (participated && pokemonDefeated) {
          ...
          if (machoBraceModifier && machoBraceModifier.stackCount < machoBraceModifier.getMaxStackCount()) {
            ...
            this.updateModifiers(true, true, expPlayerIndex);
        ...
        this.applyModifiersForPlayer(PokemonExpBoosterModifier, expPlayerIndex, partyMember, pokemonExp);
        ...
        if (exp) {
          ...
          this.phaseManager.unshiftPhase(
            expPartyMembers[pm].isOnField()
              ? this.phaseManager.create("ExpPhase", partyMemberIndex, exp, expPlayerIndex)
              : this.phaseManager.create("ShowPartyExpBarPhase", partyMemberIndex, exp, expPlayerIndex),
          );
        }
      }
    }

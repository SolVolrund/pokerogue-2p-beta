...
export class EggHatchPhase extends Phase {
  ...
  constructor(hatchScene: EggLapsePhase, egg: Egg, eggsToHatchCount: number, playerIndex: PlayerIndex = 0) {
    super();
  ...
  start() {
    ...
    this.eggLapsePhase.restoreActivePlayer();
    super.end();
  }
  ...
  doReveal(): void {
    ...
    globalScene.time.delayedCall(fixedInt(250), () => {
      ...
      globalScene.time.delayedCall(fixedInt(duration), () => {
        ...
        const gameData = globalScene.getPlayerGameData(this.playerIndex);
        const isNewStarterCatch = !gameData.dexData[rootSpeciesId].caughtAttr;

        const finishHatch = () => {
          gameData.setEggMoveUnlocked(this.pokemon.species, this.eggMoveIndex).then(value => {

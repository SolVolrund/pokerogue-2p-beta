...
export class SwitchSummonPhase extends SummonPhase {
  ...
  private getActivePartyIndex(): number {
    return this.player && globalScene.twoPlayerMode ? this.partyMemberIndex : this.fieldIndex;
  }

  private removeDuplicatePartyReferences(party: Pokemon[]): void {
  ...
  preSummon(): void {
    ...
    if (
      !this.doReturn
      || (this.slotIndex !== -1 && !(this.player ? this.getParty() : globalScene.getEnemyParty())[this.slotIndex])
    ) {
      if (this.player) {

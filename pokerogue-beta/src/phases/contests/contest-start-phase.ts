...
export class ContestStartPhase extends ContestPhase {
  ...
  start(): void {
    super.start();
    void this.startContest();
  }

  private async startContest(): Promise<void> {
    await ensureContestUiAssetsLoaded();
    await ensureContestAudioAssetsLoaded();
    ...
    globalScene.phaseManager.unshiftNew(
      ...
      this.contestState,
      this.phaseName,
      formatContestStartMessage(this.contestState),
    );

...
export class MoveEffectPhase extends PokemonPhase {
  ...
  public override start(): void {
    ...
    if (user.turnData.hitsLeft === -1) {
      ...
      globalScene.applyModifiersForPokemon(PokemonMultiHitModifier, user, user, move.id, hitCount);
  ...
  protected applyMoveDamage(user: Pokemon, target: Pokemon, effectiveness: TypeDamageMultiplier): MoveDamageTuple {
    ...
    if (user.isPlayer() && target.isEnemy()) {
      globalScene.applyModifiersForPokemon(DamageMoneyRewardModifier, user, user, new NumberHolder(finalDmg));
    }
  ...
  protected applyOnTargetEffects(
    ...
    if (this.move.is("AttackMove")) {
      globalScene.applyModifiersForPokemon(ContactHeldItemTransferChanceModifier, user, user, target);
    }
  }
  ...
  protected applyHeldItemFlinchCheck(user: Pokemon, target: Pokemon, dealsDamage: boolean): void {
    ...
    if (
      ...
      globalScene.applyModifiersForPokemon(FlinchChanceModifier, user, user, flinched);
      if (flinched.value) {
  ...
  public override end(): void {
    ...
    globalScene.applyModifiersForPokemon(HitHealModifier, user, user);
